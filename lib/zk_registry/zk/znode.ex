defmodule ZKRegistry.ZK.ZNode do
  require Logger
  use GenServer
  import ZKRegistry.ZK.Helpers
  alias ZKRegistry.ZK
  ...
  def init({type, _id} = name) when is_atom(type) do
    ...
    {:ok, created_path} = Zookeeper.Client.create(:zk_registry_connection, path_prefix, node_name, makepath: true, create_mode: :ephemeral_sequential)
  ...
  def terminate(_reason, %{name: name, znode_name: znode_name}) do
    ...
    Zookeeper.Client.delete(:zk_registry_connection, path(name, znode_name))
  end
