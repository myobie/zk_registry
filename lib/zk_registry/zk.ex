defmodule ZKRegistry.ZK do
  require Logger
  import ZKRegistry.ZK.Helpers
  alias ZKRegistry.ZK.ZNode
  ...
  def get_children({ type, _id } = name) when is_atom(type) do
    case Zookeeper.Client.get_children(:zk_registry_connection, path(name)) do
  ...
  def get_node_name({ type, _id } = name, znode_name) when is_atom(type) and is_binary(znode_name) do
    case Zookeeper.Client.get(:zk_registry_connection, path(name, znode_name)) do
      {:ok, {value, _stat}} ->
